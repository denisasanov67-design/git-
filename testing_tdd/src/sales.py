...
def _parse_record(line: str) -> dict:
    """ Parse data from one sale recotd.
     parameters:
        line: record on one sale come from file

    Return:
        data of one sale in from of dict or None if validation fails
    """
    sale =line.strip().split(",") # recording specs each must have 4 columns
    if len(sale) != 4: return None
